...
namespace Library
{
	...
	*/
	void BlockEntity::BlockToBlockCollisions(vector<std::string>& collisionInfo)
	{
		...
		for (auto& it : blocks)
		{
			//if the block we're colliding with is the same block as this instance, continue loop
			if (it == this)
			{
				continue;
			}

			//Check if this block is NOT active in the current block configuration
			...
			{
				//if the block was travelling in the upward direction (an impossible scenario)
				if (lastDirection == "Up")
				{
					//
