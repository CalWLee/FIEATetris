...
{
#if defined(DEBUG) || defined(_DEBUG)
	//Flag to check memory leaks when compiled in Debug.
	_CrtSetDbgFlag(_CRTDBG_ALLOC_MEM_DF | _CRTDBG_LEAK_CHECK_DF);
	//_crtBreakAlloc = 112;
#endif

//Outputs to console. Uses Console for Debugging the tetromino blocks as well as the opening animation cutscene.
	...
	*stdin = *hf_in;
	...
	//Worlds are effectively like scenes in Unity or Levels in Unreal.
	//main menu world
	World mainMenuWorld;
	...
	//A state to be passed around to Worlds and their descendants
	WorldState state;

	//Create a container for deserialized data
	SharedDataWorld sdw(&gameWorld);


	/** Begin Chain of Responsibility Pattern **/
	//Set Up Deserialization Framework
	XmlParseMaster m(&sdw);

	//parses the root element
	XMLParseHelperUniverse p;

	//parses specific Tetris relat
	XMLParseHelperTetris pht;
	
	//parses primitve data
	XMLParseHelperTable pt;
	
	//add above parser to master parser.
	m.AddHelper(&p);
	m.AddHelper(&pht);
	m.AddHelper(&pt);

	m.ParseFromFile("Tetris.xml");
	/** End Chain of Responsibility Pattern **/

	GameTime time;
	gameWorld.SetQueue(&q);

	//all worlds instantiated share the same state
	...
	//sets the current World being handled
